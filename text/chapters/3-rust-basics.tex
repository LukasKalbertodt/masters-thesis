...
Explanations and remarks to the code above:
...
\begin{itemize}
  ...
  One example is the assignment on line~22. While this in particular is also possible with the ternary operator \code{?:} in some other languages, Rust allows \code{match} expressions (similar to \code{switch} in \cpp), loops, full blocks and many more constructs in expression context.
...
The core language contains a number of primitive types as shown in table~\ref{tab:primitive-types} (their names are an exception to the naming convention described before).
