...
Also worth mentioning is the \enquote{Rust Playground}, an online compiler available at \url{https://play.rust-lang.org/}.
...
Used for that purpose is the generic type \code{Option<T>} from the standard library, defined as shown in figure~\ref{fig:option-result-def}.
...
Recoverable errors are handled with the \code{Result<T, E>} type whose definition can be seen in figure~\ref{fig:option-result-def}.
...
While this high-level programming looks like it involves many method calls, these iterator chains are compiled down to almost optimal machine code most of the time.
...
This restriction might initially sound too strict, but there are ,amy reasons why it is a good idea.
