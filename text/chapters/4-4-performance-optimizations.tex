...
Oftentimes, the \code{Ok} type is even \code{()}.
...
In short: the actual error is now heap allocated and \code{Error} itself is only 8 bytes large. What's more: due to null pointer optimization, \code{Result<(), Error>} is also only 8 bytes in size, meaning it can be returned from functions in a single CPU register, further improving speed.
