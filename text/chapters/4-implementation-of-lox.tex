...
But users of this library can implement the traits for their own data structures, allowing them to use many features (like mesh algorithms or IO) without \code{lox} even knowing about those types.
This is useful for highly specialized or non-mesh data structures (e.\,g. graphs).
...
\begin{figure}[th]
  ...
  \caption{
    ...
    Green boxes contain traits that deal with adjacency information, red boxes contain traits that deal with mutating a mesh.
  }
  \label{fig:mesh-traits}
\end{figure}

The current solution can be seen in figure~\ref{fig:mesh-traits}.
This, however, is not fixed at all and will likely change in the future by going through a few more design iterations.
