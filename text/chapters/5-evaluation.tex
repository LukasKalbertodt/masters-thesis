...
For example, \code{lox} is compared to the \cpp libraries OpenMesh and PMP by measuring the difference in execution speed and examining some example programs written with those libraries.
...
It needs to be noted that this DEM implementation, unlike the HEM implementation, is fairly young, meaning that it was not yet optimized in any way.
...
The reasons for these differences are not entirely clear, but profiling these benchmarks hinted at bound checks as one of the major problems for \code{lox}'s performance.
...
The performance of the directed edge mesh differs widely between benchmarks, sometimes being significantly faster than the HEMs and being equally fast in other cases.
It can clearly outperform the HEMs in the benchmarks \enquote{boundary face} and \enquote{face normals}, which makes sense, as these algorithms only use the face neighborhood.
...
And as can be seen from some benchmarks, it can be very beneficial to choose a data structure other then the HEM in some situations.
