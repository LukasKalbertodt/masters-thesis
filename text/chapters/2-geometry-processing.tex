...
The \emph{directed edge mesh} is a memory-efficient variant of the half edge mesh which only works for a fixed face valence (pure triangle or quad meshes).
...
Similarly, the face of a half edge can be found by dividing the half edge ID by 3 (rounding down).
...
The memory column specifies the memory consumption per vertex on an average triangle mesh.
From the \emph{Euler formula}, certain properties for triangle meshes can be derived (cf.~\cite{botsch2010polygon}): such a mesh contains approximately twice as many faces and thrice as many edges as it contains vertices ($|F| \approx 2|V|$ and $|E| \approx 3|V|$).
The memory consumption is thus calculated as $M = \langle\text{bytes per vertex}\rangle + 2 \cdot \langle\text{bytes per face}\rangle + 3 \cdot \langle\text{bytes per full edge}\rangle$.
